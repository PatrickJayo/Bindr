...
$ccomments_q_sql = "SELECT comment_id FROM uComments";
...
$stars_q_sql = "SELECT id FROM matches";
...
if (isset($_POST['verifyBadgeBtn'])) {
    ...
    switch (filter_var(htmlentities($_POST['verifyOptions']),FILTER_SANITIZE_STRING)) {
        ...
        if ($insertVerify) {
            $verifyBadgeResult = "<p style='color:#4CAF50;'><a href='".$directoryPath."u/".$userVerifyBadge."'> $userVerifyBadge</a> ".lang('verified_successfully')."</p>";
        }else{
        ...
        $insertVerify->execute();
        if ($insertVerify) {
            $verifyBadgeResult = "<p style='color:#4CAF50;'>".lang('verify_badge_removed_succ_from')." <a href='".$directoryPath."u/".$userVerifyBadge."'> $userVerifyBadge</a></p>";
        }else{
            $verifyBadgeResult = "<p style='color:#F44336;'> ".lang('errorSomthingWrong')."</p>";
        }
        break;
...
<div class="dashboard_box" style="text-align: <? echo lang('textAlign'); ?>">
    ...
    <table class="dashboard_UsersTable">
        <tr>
        <td style="width: 40px;">
            <div><img src="<?php echo $directoryPath; ?>imgs/user_imgs/<?php echo $rows['Userphoto']; ?>"></div>
        </td>
        <td>
            <a href="<?php echo $directoryPath; ?>u/<?php echo $rows['Username']; ?>"><p><?php echo $rows['Fullname']; ?></p></a>
        </td>
    ...
    <table class="dashboard_sbFetch">
        ...
        <tr>
            ...
            <td><a href="<? echo $directoryPath.'u/'.$fromid_un; ?>"><? echo $fromid_name; ?></a></td>
