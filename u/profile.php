...
<head>
    <title><?php echo $row_username; ?> | Bindr</title>
    <meta charset="UTF-8">
...
<!--=============================[ Container ]=====================================-->
        ...
        <div class="profile_menu">
            <div class="profile_menu_details" style="display:inline-flex;">
                ...
                //=====================================================================
                $matches_num_sql = "SELECT id FROM matches WHERE p_id=:row_id";
                $matches_num = $conn->prepare($matches_num_sql);
                $matches_num->bindParam(':row_id',$row_id,PDO::PARAM_INT);
                $matches_num->execute();
                ...
                <a href='<?php echo $row_username;?>&ut=matches' id='posts_btn' class='profileMenuItem'>
                <b><?php echo thousandsCurrencyFormat($matches_num_int); ?></b> <?php echo lang('matches_str'); ?></a>
        ...
        <div class="profile_picture">
            ...
            <?php
               if($row_id != $_SESSION['id']){
                ...
                $matchesCheck = $conn->prepare($sql);
                $matchesCheck->bindParam(':uid',$myId,PDO::PARAM_INT);
                $matchesCheck->bindParam(':pid',$row_id,PDO::PARAM_INT);
                $matchesCheck->execute();
                $matchesCheckExist = $matchesCheck->rowCount();
                if ($matchesCheckExist > 0) {
                echo "<span id='rate_star'><button class='follow_btn' onclick='matchPage(\"$myId\",\"$row_id\")' style='width:100%;margin:0px 3px;border-color:#ffc107;padding:10px 15px;' title='".lang('unFavoritePage')."'><span class='fa fa-heart' style='color:#FFC107;font-size:18px;'></span></button></span>";
                
                }else{
                echo "<span id='rate_star'><button class='follow_btn' onclick='matchPage(\"$myId\",\"$row_id\")' style='width:100%;margin:0px 3px;padding:10px 15px;' title='".lang('addToFavoritePages')."'><span class='fa fa-heart-o' style='color:#bbbbbb;font-size:18px;'></span></button></span>";
                }
                }
                ?>
                </div>
...
<?php
$s_id = $_SESSION['id'];
$getS_sql = "SELECT * FROM matches WHERE p_id =:row_id";
...
while ($getS_row = $getS->fetch(PDO::FETCH_ASSOC)) {
    ...
    while ($getuser_row = $getuser->fetch(PDO::FETCH_ASSOC)) {
        $id_matches = $getuser_row['id'];
        $fullname_matches = $getuser_row['Fullname'];
        ...
        $followBtn_num = $followBtn->rowCount();
        if ($followBtn_num > 0){
            $follow_btn = "<span id='followUnfollow_$id_matches' style='cursor:pointer'><button class=\"unfollow_btn\" onclick=\"followUnfollow('$id_matches')\"><span class=\"fa fa-check\"></span> ".lang('followingBtn_str')."</button></span>";
        }else{
            $follow_btn = "<span id='followUnfollow_$id_matches' style='cursor:pointer'><button class=\"follow_btn\" onclick=\"followUnfollow('$id_matches')\"><span class=\"fa fa-plus-circle\"></span> ".lang('followBtn_str')."</button></span>";
        }
        ...
        if($id_matches != $_SESSION['id']){
               echo "
        <table class='user_follow_box' id='UserUnfollow_$id_matches'>
        <tr>
        <td class='user_info_tdi'><div><img src=\"../imgs/user_imgs/$userphoto_matches\" alt=\"$fullname_matches\" /></div></td>
        <td style='width: 70%;'><a href=\"$username_matches\" class='user_follow_box_a'><p>$fullname_matches $verifypage_var<br><span style='color:gray;'>@$username_matches</span></a></td>
        <td style='width: 100%;'><span style='float:".lang('float2').";'>$follow_btn</span></td>
        </tr>
        </table>
        ";
