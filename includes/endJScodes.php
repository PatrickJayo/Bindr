...
function matchPage(uid, pid){
$.ajax({
    type:"POST",
    url: "<?php echo $check_path; ?>includes/match.php",
    data:{'uid':uid,'pid':pid},
    beforeSend:function(){
        $('#rate_star').html("<button class='follow_btn' onclick='matchPage(\"$uid\",\"$pid\")' style='width:100%;margin:0px 3px;border-color:#ffc107;padding:10px 15px;' title='<?php echo lang('unmatch'); ?>'><span class='fa fa-heart' style='color:#FFC107;font-size:18px;'></span></button>");
    },
    success:function(data){
        $('#rate_star').html(data);
    },
